...
export default function Dashboard() {
  ...
  // ▶ On mount: restore session & scheme from localStorage, then fetch students
  useEffect(() => {
    const savedSession = localStorage.getItem('sessionId');
    if (savedSession) {
      setSessionId(savedSession);

      const schemeJson = localStorage.getItem(`markingScheme_${savedSession}`);
      if (schemeJson) {
        setMarkingScheme(JSON.parse(schemeJson));
      }

      // fetch students
      api
        .get('/get-session-results', { params: { session_id: savedSession } })
        .then((res) => {
          const list = res.data.students.map((s) => ({
            studentId: s.student_id,
            studentName: s.student_name,
  ...
  // ▶ Create a new session (also overwrite localStorage)
  const createSession = () => {
    api
      .post('/create-session')
      .then((res) => {
        ...
        // persist
        localStorage.setItem('sessionId', id);
        localStorage.removeItem(`markingScheme_${id}`); // clear any old scheme
      })
      .catch(console.error);
  };

  // ▶ Delete the current session (also clear localStorage)
  const deleteSession = () => {
    if (!sessionId) return;
    api
      .delete('/delete-session', { params: { session_id: sessionId } })
  ...
  // ▶ Upload question paper & persist markingScheme
  const handleQuestionUpload = async (e) => {
    e.preventDefault();
    if (!questionFile || !sessionId) return;
    const fd = new FormData();
    fd.append('file', questionFile);
    fd.append('session_id', sessionId);
  ...
  // ▶ Upload student PDF & refresh list
  const handleStudentUpload = async (e) => {
    e.preventDefault();
    if (!studentFile || !studentName || !studentId || !sessionId) return;
  ...
  // ▶ Navigate to student result page
  const openResult = (student) =>
    nav('/student-result', { state: { student } });
    ...
    <div className="dashboard-container">
      ...
      {sessionId && (
        <>
          <section className="upload-section">
            <h2>1. Upload Question Paper</h2>
            <form onSubmit={handleQuestionUpload} className="upload-form">
              <input
                type="file"
                accept="application/pdf"
            ...
            {markingScheme && (
              <button onClick={viewScheme} className="btn view-btn">
                View Marking Scheme
              </button>
            )}
          </section>
          ...
          <section className="upload-section">
            <h2>2. Upload Student Submission</h2>
            <form onSubmit={handleStudentUpload} className="upload-form">
              <input
                type="text"
                placeholder="Student Name"
                value={studentName}
